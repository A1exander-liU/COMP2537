...
app.get("/pokedex", loginValidator, function(req, res) {
...
app.delete("/removeGameResult", function(req, res) {
    userModel.updateOne({username: req.session.current_user[0].username}, {$pull: {game_log: {date: req.body.date}}}, function(err, data) {
        if (err) {
            console.log("Err" + err)
        }
        else {
